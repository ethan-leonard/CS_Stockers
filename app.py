...
def buy():
    ...
    """Buy shares of stock"""
    if request.method == "GET":
        if not session.get("bypass_market_check"):
            info = lookup("cost")
            if info["marketStatus"] == False:
                return render_template("closed.html", route='buy')
        else:
...
def sell():
    ...
    if request.method == "GET":
        ...
        shares2sell = request.form.get("shares")

        if not shares2sell.isdigit():
            return apology("Shares Must Be Whole Number")

        shares2sell = int(shares2sell)
...
@login_required
def bypass():
    session["bypass_market_check"] = True
